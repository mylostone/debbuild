...
git show "${latest_tag}:$project.spec" > "$HOME/debbuild/SPECS/$project.spec"

./configure
make

set +e
./debbuild.out -bb "$HOME/debbuild/SPECS/$project.spec"
RET=$?
if [ $RET -eq 0 ]; then
	outf=$(ls -1 $HOME/debbuild/DEBS/all/$project_${latest_tag}*.deb)
